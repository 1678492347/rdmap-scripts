...
# 找出所有disorder
def find_disorder():
    with open('/Users/yj/IdeaProjects/RD_Map/rd_map/python_code/origin_data/all_disorder.csv', 'r') as all_disorder1:
...
def find_orpha_hpo():
    with open('/Users/yj/IdeaProjects/RD_Map/rd_map/python_code/origin_data/all_orpha_hpo.csv', 'r', encoding='UTF-8-sig') as all_orpha_hpo1:
...
def find_hpo_index():
    global hpo_index
    with open('/Users/yj/IdeaProjects/RD_Map/rd_map/python_code/origin_data/part_dis/index.csv', 'r') as file:
        reader = csv.reader(file)
        rows = [row for row in reader]
        for a in rows:
            hpo_index.append(a)
    with open('/Users/yj/IdeaProjects/RD_Map/rd_map/python_code/origin_data/new_dis/hpo_index.csv', 'r') as file:
        reader = csv.reader(file)
        rows = [row for row in reader]
        for a in rows:
            hpo_index.append(a)
...
def disorder_hpo_dic():
    relationdic = dict()
    term = []
    with open('/Users/yj/IdeaProjects/RD_Map/rd_map/python_code/origin_data/disordertohpo(no0)sorted1.csv', 'r') as csvfile:
...
if __name__ == '__main__':
    ...
    hpo_group = list(set(sys.argv[2:]))
    # hpo_group = ['HP:0000256', 'HP:0000035', 'HP:0003537']
    # hpo_group = ['HP:0003537']
    max_dis = int(sys.argv[1])
    # hpo_group = ['HP:0001250', 'HP:0003537']
    ...
    for i in list(set(file_index)):
        if i < 11:
            ...
            part_distance = pd.read_csv('/Users/yj/IdeaProjects/RD_Map/rd_map/python_code/origin_data/new_dis/new_hpo_distance_a' + str(i-11) + '.csv', index_col=0,
                                        header=0)
            convert_part_distance = part_distance.apply(pd.to_numeric, downcast='unsigned')
            hpo_distance2 = pd.concat([hpo_distance2, convert_part_distance], axis=1)

    result = []
    for i in range(len(hpo_group)):
        if file_index[i] < 11:
            sorted_hpo_dis = hpo_distance1[hpo_group[i]].nsmallest(len(hpo_distance1.index))
        else:
            sorted_hpo_dis = hpo_distance2[hpo_group[i]].nsmallest(len(hpo_distance2.index))

        for hpo in sorted_hpo_dis.index.values:
            if sorted_hpo_dis[hpo] <= max_dis:
                result.append(hpo)
            else:
                break
    # print(result)
